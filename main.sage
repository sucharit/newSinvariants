...
def complexes(groups,maps, ras,print_progress = True):
    """
    ...
    range_min=0#Hack to compute Kh only in a single q=ras-1
    range_max=len(temp.keys())
    if ras!=None:
        range_min=temp.keys().index(ras-1)
...
class KnotKh():
    ...
    def __init__(self, data, (nplus,nminus)=(0,0), ras=None, print_progress = True):
        ...
        (self.complexes,self.newhom)=complexes(self.groups,self.maps,ras,print_progress)
        if print_progress:
            print "Time taken is "+repr(time.time()-start_time)
...
############ PD diagram to our format #########################################

def PDtoOur(data):
    #converts PD presentation to a tuple (a,b). a is 0-resolution and b is (n+,n-).
    
    circles=[]
    crossings=flatten(data)
    ...
    res = [CycList(x) for x in circles]

    [nplus,nminus]=[0,0]
    for j in range(len(data)):
        [a,b]=[data[j][1],data[j][3]]
        if abs(a-b)==1:
            if a>b:
                nplus=nplus+1
            if a<b:
...
def PDtoCan(data):
    #converts PD presentation to the oriented resolution vertex.
...
def higher_chain_maps(group,edges, print_progress = True):
    """
    Input: the output of chain_group and the output of populate_edges
    Output: A dictionary whose keys are pairs of integers (t,q), and the key is a matrix which describes the map (Bar-Natan differential) from the chain group at (t,q) to the chain group at (t+1,q+2).
    This might be the slowest step!!
    Matrix acts like: input row vector times matrix = output row vector. (v\cdot M) (Sage convention!!)
    """
    maps = dict()
    ...
    for k in group.keys():
        if print_progress:
            print repr(1+group.keys().index(k))+'/'+repr(len(group.keys()))
        l=(k[0]+1,k[1]+2)#Added l to make stuff more human-readable. SS
        if l in group.keys():
            maps[k]=matrix(GF(2),len(group[k]),len(group[l]),sparse=True)
    #Now, add 1's for corresponding to the edge maps...
    #RL just wrote this, and has done zero testing so far. Promises to debug later.
...
def combined_maps(groups,maps,higher_maps, min_q=None,print_progress = True):
    """
    If min_q=None, combines maps and higher maps into a single matrix.
    Or else only does it for the subcomplex q>=min_q.
    Returns the matrix from -1 to 0 (ans[-1]), and the one from 0 to 1 (ans[0]).
    """
    ans=dict()
    
    if min_q==None:
        min_q=min(Set([q for (t,q) in groups.keys()]))

    max_q=max(Set([q for (t,q) in groups.keys()]))

    ranks=dict()
    for i in range(-1,2):#only computing near 0
        s=dict({min_q-2:0})
        
        for q in range(min_q,max_q+1,2):
            if (i,q) in groups.keys():
                s[q]=s[q-2]+len(groups[(i,q)])
            else:
                s[q]=s[q-2]
        ranks[i]=s# sums of ranks up to this point

    for i in range(-1,0):#in the old program, computing ans[-1] and ans[0]. Here only ans[-1]. This is just for consistency.
        ans[i]=matrix(GF(2),ranks[i][max_q],ranks[i+1][max_q],sparse=True)

        #Now add entries
        for q1 in range(min_q,max_q+1,2):
            if print_progress:
                print (q1,min_q,max_q)
            for q2 in range(q1,min(q1+2,max_q)+1,2):
                for a1 in range(ranks[i][q1]-ranks[i][q1-2]):
                    for a2 in range(ranks[i+1][q2]-ranks[i+1][q2-2]):
                        if q1==q2:
                            ans[i][ranks[i][q1-2]+a1,ranks[i+1][q2-2]+a2]=maps[(i,q1)][a1,a2]
                        else:
                            ans[i][ranks[i][q1-2]+a1,ranks[i+1][q2-2]+a2]=higher_maps[(i,q1)][a1,a2]
...
class RasmusKh(KnotKh):
    def __init__(self, data, oriented_v,(np,nm)=(0,0),rasmus=0, print_progress = True):

        KnotKh.__init__(self,data,(np,nm),rasmus,print_progress=print_progress)

        self.rasmus=rasmus
        self.oriented_v=oriented_v
        self.oriented_res=self.vertices[tuple(self.oriented_v)]

        start_time=time.time()
        if print_progress:
            print "Finding the higher maps."
        self.higher_maps = higher_chain_maps(self.groups,self.edges, print_progress)
        if print_progress:
            print "Time taken is "+repr(time.time()-start_time)

        self.combined=combined_maps(self.groups,self.maps,self.higher_maps,None,print_progress)




        #C_0(F_q) maps to C_0(C) by matrix A (a x b)
        #C_-1(C) maps to C_0(C) by matrix B (c x b)
        #C_0(F_q) maps to KC^0,q by matrix C (a x d)
        #KC^-1,q maps to KC^0,q by matrix D (e x d)
        #Kh^-2,q maps to KC^0,q by matrix E (f x d)
        #We have a cycle cyc in C_0(C)
        #dimensions of C_0(F_q), C_0(C), C_-1(C), KC^0,q, KC^-1,q and Kh^-2,q
        #are a,b,c,d,e,f.

        #Big matrix=M (a+c+e+f x b+d), Big cycle=X (b+d)

        self.a=sum([len(self.groups[(t,q)]) for (t,q) in self.groups.keys() if t==0 and q>=self.rasmus-1])
        self.b=sum([len(self.groups[(t,q)]) for (t,q) in self.groups.keys() if t==0])
        self.c=sum([len(self.groups[(t,q)]) for (t,q) in self.groups.keys() if t==-1])
        self.d=len(self.groups[(0,self.rasmus-1)])
        self.e=len(self.groups[(-1,self.rasmus-1)])
        self.f=len(self.newhom[self.rasmus-1][-2][1])

        if print_progress:
            print "Generating the final matrix. "+repr((self.a+self.c+self.e+self.f,self.b+self.d))


        self.A=matrix(GF(2),self.a,self.b,sparse=True)
        for i in range(self.a):
            self.A[i,self.b-self.a+i]=1


        self.B=self.combined[-1]

        self.C=matrix(GF(2),self.a,self.d,sparse=True)
        for i in range(self.d):
            self.C[i,i]=1

        self.D=self.maps[(-1,self.rasmus-1)]

        rows=[]
        for i in range(1,self.f+1):
            rows.append(self.sq2((-2,self.rasmus-1),n=i,print_progress=print_progress,show_chain=True)[0])
        self.E=matrix(GF(2),self.f,self.d,rows,sparse=True)


        self.M=block_matrix(GF(2),4,2,[self.A,self.C,self.B,0,0,self.D,0,self.E],sparse=True,subdivide=False)
 
        

        if print_progress:
            print "Generating the canonical cycle."

        self.cyc=self.find_canonical()

        self.X=vector(GF(2),self.cyc+self.d*[0,],sparse=True)

        if print_progress:
            print "Solving the linear algebra problem."


        try:
            self.M.solve_left(self.X)
            self.is_more=True
        except ValueError:
            self.is_more=False
    ...
    def find_canonical(self):
        ...
        qmin=min([q for (t,q) in self.groups.keys() if t==0])
        qmax=max([q for (t,q) in self.groups.keys() if t==0])

        for i in range(qmin,qmax+2,2):
            if (0,i) in self.groups.keys():
                hom_0_gens+=self.groups[(0,i)]


        #find the bipartite graph from the vertices of self.oriented_res
        piece1=Set([0,])
        piece2=Set([])

        while piece1+piece2 != Set(range(self.oriented_res.C)):
            for e in self.oriented_res.edge_incidences.keys():
                [z1,z2]=self.oriented_res.edge_incidences[e]
        ...
        for x in hypercube(self.oriented_res.C)[0]:
            #x on piece1 circles evaluate to 1

            faulty=[fault for fault in piece1 if x[fault]==0] 

            if faulty==[]:
                ans[hom_0_gens.index((tuple(self.oriented_v),x))]=1
